...
export async function createMap(e: HTMLDivElement, search = true): Promise<MapView> {
  mapView.container = e;
  if (search) {
...
export async function fetchMeteostatData(latitude: number, longitude: number): Promise<IMeteostatData[]> {
  const fetchResponse = await fetch(`${METEOSTAT_ENDPOINT}?lat=${latitude}&lon=${longitude}&units=metric&start=1991&end=2020`, options)
...
export function calculateModel(monthTemperature: number): ComputedRef<number> {
  return computed(() => {
    // const HRT = ((inputParameters.A * inputParameters.h) / inputParameters.Q) * ev;
    const HRT = ((inputParameters.A * inputParameters.h) / inputParameters.Q);
    const r = ((inputParameters.Ci - Cb) / (1 + (((k20 * (theta ** (monthTemperature - 20))) / (inputParameters.h * ev)) * HRT / inputParameters.N)) ** inputParameters.N) + Cb
